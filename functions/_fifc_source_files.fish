function _fifc_source_files -d "Return a command to recursively find files"
    ...
    if type -q fd
        ...
        if test "$path" = {$PWD}/
            echo "fd . $fifc_fd_opts --max-depth 1 --color=always $hidden_flag $fd_custom_opts"
        else if test "$path" = "."
            echo "fd . $fifc_fd_opts --max-depth 1 --color=always $hidden_flag $fd_custom_opts"
        else if test -n "$hidden"
            echo "fd . $fifc_fd_opts --max-depth 1 --color=always $hidden_flag -- $path"
        else
            echo "fd . $fifc_fd_opts --max-depth 1 --color=always $hidden_flag -- $path"
        end
    else if test -n "$hidden"
        # Use sed to strip cwd prefix
        echo "find . $path -maxdepth 1 $fifc_find_opts ! -path . -print 2>/dev/null | sed 's|^\./||'"
    else
